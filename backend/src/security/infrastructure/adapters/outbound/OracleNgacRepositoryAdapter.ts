...
export class OracleNgacRepositoryAdapter implements NgacRepositoryPort {
  /**
   * @function    fetchNodes
  ...
  async fetchUserContext(username: string): Promise<UserContext | null> {
    ...
    try {
      ...
      const result = await connection.execute(sql, { slug: normalizedSlug }, { outFormat: oracledb.OUT_FORMAT_OBJECT });
      const row = (result.rows && result.rows[0]) as any;
