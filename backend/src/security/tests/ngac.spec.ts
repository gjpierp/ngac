...
import { NgacRepositoryPort, UserContext, AccessLogEntry } from "../application/ports/outbound/NgacRepositoryPort";
import { NgacGraphManager } from "../application/services/NgacGraphManager";
import { AccessEvaluator } from "../application/services/AccessEvaluator";

class MockNgacRepository implements NgacRepositoryPort {
  public loggedEntries: AccessLogEntry[] = [];

  async fetchNodes(): Promise<DomainNode[]> {
    return [
  ...
  async fetchAssignments(): Promise<DomainAssignment[]> {
    return [
  ...
  async fetchAssociations(): Promise<DomainAssociation[]> {
    return [
  ...
  async fetchProhibitions(): Promise<DomainProhibition[]> {
    return [
  ...
  async fetchMenuNodes(): Promise<DomainMenuNode[]> {
    return [
      { idMenuNode: 1001, idNode: 6, code: "OA_HISTORIALES", label: "Menú Historiales", displayOrder: 1, active: true }
    ];
  }

  async fetchMenuAssignments(): Promise<DomainMenuAssignment[]> {
    return [];
  }

  async fetchPolicyMenuRoots(): Promise<DomainPolicyMenuRoot[]> {
    return [
      { policyId: 8, menuNodeId: 1001 }
    ];
  }

  async fetchUserContext(username: string): Promise<UserContext | null> {
    if (username === "gerardo") {
      return {
        userId: 10,
...
async function runTests() {
  ...
  assert.strictEqual(graph.nodes.size, 8, "Deberían haberse cargado 8 nodos");
  ...
  assert.ok(userAncestors.has(1), "Ancestros deben incluir al propio nodo");
  assert.ok(userAncestors.has(2), "Ancestros deben incluir ROL_ADMIN");
  assert.ok(userAncestors.has(3), "Ancestros deben incluir ROL_MEDICO");
  assert.ok(userAncestors.has(8), "Ancestros deben incluir PC_SALUD (herencia transitiva)");
  ...
  assert.strictEqual(hasAccessVer, 1, "Debería otorgar acceso VER por asociación de ROL_MEDICO");
  ...
  assert.strictEqual(hasAccessEditarOk, 0, "Debería denegar EDITAR debido a la prohibición en UNI_URGENCIA (Deny-Overrides)");
  ...
  const hasAccessEditarDirecto = await evaluator.verificarAcceso(
    ["ROL_ADMIN", "ENT_HOSPITAL"], // Sin UNI_URGENCIA
    ["EDITAR"],
    "OBJ_FICHA_101",
    { sujeto: { division: "URGENCIA" } }
  );
  assert.strictEqual(hasAccessEditarDirecto, 1, "Debería autorizar EDITAR si no tiene la prohibición y la condición ABAC se cumple");

  const hasAccessEditarCondFalla = await evaluator.verificarAcceso(
    ["ROL_ADMIN", "ENT_HOSPITAL"],
    ["EDITAR"],
    "OBJ_FICHA_101",
    { sujeto: { division: "OTRA" } }
  );
  assert.strictEqual(hasAccessEditarCondFalla, 0, "Debería denegar EDITAR si la condición ABAC de división no se cumple");
  ...
  assert.ok(menuResult.menu, "El resultado debería contener el menú");
  assert.strictEqual(menuResult.menu.length, 1, "Debería retornar exactamente 1 menú raíz");
  assert.strictEqual(menuResult.menu[0].codigo, "OA_HISTORIALES", "El menú retornado debe ser el permitido");
