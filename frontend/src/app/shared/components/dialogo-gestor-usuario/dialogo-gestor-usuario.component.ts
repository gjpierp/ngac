...
@Component({
  ...
  template: `
    <div
      class="rounded-3xl overflow-hidden shadow-2xl bg-white max-w-xl w-full border border-gray-150 animate-in zoom-in-95 duration-200"
    >
      <!-- Cabecera Premium del Gestor -->
      <div class="p-6 bg-slate-900 text-white relative flex justify-between items-start">
        <div
          class="absolute inset-0 bg-gradient-to-r from-slate-900 to-indigo-950 opacity-95 z-0"
        ></div>

        <div class="relative z-10">
          <span
            class="text-[9px] font-black uppercase tracking-wider px-2.5 py-0.5 bg-indigo-500/25 border border-indigo-400/30 rounded-md text-indigo-300"
          >
            ID SAFI: {{ usuario.id }}
          </span>
        ...
        <button
          mat-icon-button
          (click)="close()"
          class="relative z-10 text-slate-400 hover:text-white hover:bg-slate-800/50 rounded-xl transition-all"
        >
      ...
      <div class="p-6 space-y-6 max-h-[460px] overflow-y-auto">
        ...
        <div class="space-y-3">
          <h4
            class="text-[11px] font-black uppercase tracking-widest text-slate-450 flex items-center gap-2"
          >
          ...
          <div
            class="flex flex-wrap gap-2 min-h-[44px] p-3 bg-slate-50 border border-slate-100 rounded-2xl"
          >
            <div
              *ngFor="let rol of assignedRoles()"
            ...
            <div
              *ngIf="assignedRoles().length === 0"
              class="text-gray-400 text-xs italic py-1 pl-1"
            >
        ...
        <div class="space-y-2.5">
          ...
          <div class="relative">
            ...
            >
              <mat-option [value]="null" class="text-red-600 font-black"
                >❌ Sin Entidad Comercial (Desvincular)</mat-option
              >
        ...
        <div class="space-y-2.5">
          ...
          <div class="relative">
            ...
            >
              <mat-option [value]="null" class="text-red-600 font-black"
                >❌ Sin Unidad Organizacional (Desvincular)</mat-option
              >
      ...
      <div
        class="bg-slate-50/80 p-4 border-t border-slate-100 flex items-center justify-between text-[9px] uppercase font-black tracking-widest text-slate-400"
      >
        <span>Relational Integrity Engine</span>
        <span
          class="flex items-center gap-1.5"
          [ngClass]="loading() ? 'text-amber-500' : 'text-emerald-500'"
        >
          <span
...
export class DialogoGestorUsuarioComponent implements OnInit {
  ...
  assignedRoles = computed<RoleItem[]>(() => {
    ...
    return this.roles()
      ...
      .map((r) => ({ id: r.id_nodo, label: r.etiqueta }));
  });

  availableRoles = computed<RoleItem[]>(() => {
    const userNode = this.findUserNode();
    if (!userNode) return this.roles().map((r) => ({ id: r.id_nodo, label: r.etiqueta }));
  ...
  cargarDatos() {
    ...
    this.safiSvc.getUsuarioVinculos(this.usuario.id).subscribe({
      next: (res) => {
        if (res.success && res.data) {
          const eId =
            res.data.entidadIds && res.data.entidadIds.length > 0 ? res.data.entidadIds[0] : null;
          const uId =
            res.data.unidadIds && res.data.unidadIds.length > 0 ? res.data.unidadIds[0] : null;
    ...
    this.safiSvc.getEntidades().subscribe((res: ISafiEntidad[]) => {
      this.entidades = (res || []).filter((e: ISafiEntidad) => e.activo !== 'N');
    });

    this.safiSvc.getUnidades().subscribe((res: ISafiUnidad[]) => {
      this.unidades = (res || []).filter((u: ISafiUnidad) => u.activo !== 'N');
    });
  ...
  onEntityChange(newEntityId: number | null) {
    ...
    const completeChange = () => {
      if (newEntityId) {
        this.safiSvc
          .vincularUsuarioEntidad({ id_usuario: this.usuario.id, id_entidad: newEntityId })
          .subscribe({
            next: () => {
    ...
    if (oldEntityId) {
      this.safiSvc
        .desvincularUsuarioEntidad({ id_usuario: this.usuario.id, id_entidad: oldEntityId })
        .subscribe({
          next: () => completeChange(),
  ...
  onUnitChange(newUnidadId: number | null) {
    ...
    const completeChange = () => {
      if (newUnidadId) {
        this.safiSvc
          .vincularUsuarioUnidad({ id_usuario: this.usuario.id, id_unidad: newUnidadId })
          .subscribe({
            next: () => {
    ...
    if (oldUnidadId) {
      this.safiSvc
        .desvincularUsuarioUnidad({ id_usuario: this.usuario.id, id_unidad: oldUnidadId })
        .subscribe({
          next: () => completeChange(),
          error: () => completeChange(),
        });
  ...
  assignRole(roleId: number) {
    ...
    this.loading.set(true);
    this.accesosSvc.enlazarNodos(roleId, userNode.id_nodo).subscribe({
      next: () => {
  ...
  revokeRole(roleId: number) {
    ...
    this.loading.set(true);
    this.accesosSvc.deleteEnlace(roleId, userNode.id_nodo).subscribe({
      next: () => {
